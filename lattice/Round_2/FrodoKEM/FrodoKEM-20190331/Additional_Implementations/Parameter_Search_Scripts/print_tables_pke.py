...
def security_to_tex(p, print_sec=True):
  ...
  samples = 2 * n * (p['n_bar'] + p['m_bar']) + p['n_bar'] * p['m_bar']
  q = 2 ** qlog
  ret = ''
  ...
  for cost in [primal_cost, dual_cost]:
    ...
    ret += r'{} & {} \\'.format(int(cost_pc), int(cost_pp)) + '\n'
...
def distribution_to_python(p):
  s = '{'
  for v in sorted(p['distr']):
    if v == 0:
      s += '{}: {:.15f},'.format(v, p['distr'][v])
    else:
      s += ' {}: {},'.format(v, p['distr'][v] / 2)
      s += ' {}: {},'.format(-v, p['distr'][v] / 2)
  s += '}'
  return s
...
def main():
  # pyformat: disable
  parameters = [
    {'name': 'Frodo-640', 'sigma': 2.8, 'n': 640, 'm_bar':8, 'n_bar':8, 'q': 15, 'B': 2, 'key_len': 128, 'rand_bits': 16, 'sec_base': 105},
    {'name': 'Frodo-976', 'sigma': 2.3, 'n': 976, 'm_bar': 8, 'n_bar': 8, 'q': 16, 'B': 3, 'key_len': 192, 'rand_bits': 16, 'sec_base': 151},
    {'name': 'Frodo-1344', 'sigma': 1.4, 'n': 1344, 'm_bar': 8, 'n_bar': 8, 'q': 16, 'B': 4, 'key_len': 256, 'rand_bits': 16, 'sec_base': 195},
  ]
  # pyformat: enable
  ...
  for p in parameters:
    print('sigma = {:.2f}: {}'.format(p['sigma'], distribution_to_python(p)))
  print()
