...
_INLINE_ uint32_t sha_prepare_last_block(OUT uint8_t *last_block,
                                         ...
                                         IN const uint32_t message_len)
{
    uint32_t i;
    const uint32_t loc = message_len - (message_len % HASH_BLOCK_SIZE);
    const uint32_t ret = ((message_len % HASH_BLOCK_SIZE) <= SLICE_REM) ? 1 : 2;
    const uint32_t last_len = ret * HASH_BLOCK_SIZE;
    const uint64_t encoded_len = bswap_64((uint64_t)message_len*8ULL);

    for(i=0; i < message_len % HASH_BLOCK_SIZE; i++)
    {
    ...
    //((uint64_t*)last_block)[(last_len/8)-1] = mlen;
    memcpy(&last_block[last_len-8], &encoded_len, sizeof(uint64_t));
                           ...
                           IN const uint32_t len)
{
    ...
    for (i=0; i<num_buffers-1; i++) 
    {
        ...
        hdesc[i].blocks = sha_prepare_last_block(lastblock[i], in+i*block_len, block_len);
    }

    hdesc[i].ptr = lastblock[num_buffers-1];
    hdesc[i].blocks = sha_prepare_last_block(lastblock[num_buffers-1], 
                      in+i*block_len, block_len+(len%num_buffers));      
            ...
            IN const uint32_t num)
{
    ...
    sha_mb_init  (&ctx,num);
    sha_mb_update(&ctx, msg, byte_len, num);
    sha_mb_final (hash_out, &ctx,num, msg, byte_len);
}
