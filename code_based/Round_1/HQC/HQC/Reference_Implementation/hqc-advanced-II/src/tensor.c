...
void tensor_code_encode(vector_u32* em, vector_u32* m) {
	...
	#ifdef VERBOSE
    printf("\n\nBCH code word: "); vector_u32_print(c, UTILS_BCH_CODEWORD_ARRAY_SIZE);
    printf("\n\nTensor code word: "); vector_u32_print(tmp, UTILS_TENSOR_CODEWORD_ARRAY_SIZE);
  #endif
...
void tensor_code_decode(vector_u32* m, vector_u32* em) {
	...
	#ifdef VERBOSE
    printf("\n\nBCH input code word: "); vector_u32_print(c, UTILS_BCH_CODEWORD_ARRAY_SIZE);
  #endif
	vector_u32_clear(c);	
}