 ...
 * of the array of this structure contain an element of the support 
 ...
 * the other one stored by coordinate. The vector <b>v1</b> must be stored by position and
 * the vector <b>v2</b> must be stored by coordinate.
 * For more details on this algorithm see the <a href="../doc_mul.pdf" target="_blank"><b>multiplication algorithm</b></a>.
 *
 ...
 * \brief Generates a random vector of a given Hamming weight
 *
 * This function generates uniformly at random a binary vector of a Hamming weight equal to weight. The vector 
 ...
 *
 * The parameter \f$ t \f$ is computed for all the possible values of PARAM_N and it's denoted by UTILS_REJECTION_THREESHOLD (see the file parameters.h).
 *
 * \param[in] v a pointer to a vector
 * \param[in] weight a uint16_t that is the Hamming weight
 ...
 *
 * This function generates a random binary vector stored by coordinate. First, it generates a random 
 * array of bytes using the seedexpander function. Then, since the parameter N (PARAM_N) is not a multiple of 32, 
 * we set to zero the extra bits in the last element of v->value. We achieve this, by using a mask (UTILS_MASK), 
 * that is computed with respect to each parameter N of the scheme. 
 *
 * \param[in] v a pointer to a vector 
 * \param[in] ctx a pointer to the context of the seed expander
 */
