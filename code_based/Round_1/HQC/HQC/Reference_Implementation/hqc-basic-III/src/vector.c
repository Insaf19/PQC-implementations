...
void vector_u32_fixed_weight(vector_u32* v, const uint16_t weight, AES_XOF_struct* ctx) {
	...
	for (uint32_t i = 0; i < weight ; ++i)	{
		...
		} while(random_data >= UTILS_REJECTION_THREESHOLD);
...
void vector_u32_print(vector_u32* v, int param) {
	...
	if(param == PARAM_N) {
		...
		print_bytes(v->value[v->dim - 1], (PARAM_N % 32) / 8 + 1);
	}
	...
	if (param == UTILS_BCH_CODEWORD_ARRAY_SIZE) {
		for (int i = 0; i < (param-1); ++i)	{
			print_bytes(v->value[i], 4);
		}
		print_bytes(v->value[v->dim - 1], (PARAM_N1 % 32) / 8 + 1);
	}

	if (param == UTILS_TENSOR_CODEWORD_ARRAY_SIZE) {
		for (int i = 0; i < (param-1); ++i)	{
			print_bytes(v->value[i], 4);
		}
		print_bytes(v->value[v->dim - 1], (PARAM_N % 32) / 8 + 1);
...
void print_bytes(uint32_t value, int size) {
	for (int i = 0; i < size; ++i)	{ //size max = 4
