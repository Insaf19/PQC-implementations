...
/**
  * \file kem.c
  * \brief Implementation the api.h
  */
...
/**
  ...
  *
  * The public key is composed of the syndrom s as well as the seed used to generate vectors g and qr.
  *
...
/**
  *\fn int crypto_kem_enc(unsigned char* ct, unsigned char* ss, unsigned char* pk)
...
int crypto_kem_enc(unsigned char* ct, unsigned char* ss, const unsigned char* pk) {
  ...
  #ifdef VERBOSE
    printf("\n\npk: "); for(int i = 0 ; i < PUBLIC_KEY_BYTES ; ++i) printf("%x", pk[i]);
    printf("\n\nm: "); vector_u32_print(m, PARAM_K);
    printf("\n\ntheta: "); for(int i = 0 ; i < SEED_BYTES ; ++i) printf("%x", theta[i]);
  #endif
  ...
  #ifdef VERBOSE
    printf("\n\nd: "); for(int i = 0 ; i < SHA512_BYTES ; ++i) printf("%x", d[i]);
    printf("\n\nciphertext: "); for(int i = 0 ; i < CIPHERTEXT_BYTES ; ++i) printf("%x", ct[i]);
    printf("\n\nsecret 1: "); for(int i = 0 ; i < SHARED_SECRET_BYTES ; ++i) printf("%x", ss[i]);
  #endif
...
/**
  *\fn int crypto_kem_dec(unsigned char* ss, unsigned char* ct, unsigned char* sk)
  *\brief Decapsulation of the HQC_KEM IND_CAA2 scheme
  *
  * \param[out] ss String containing the shared secret
  * \param[out] ct String containing the cipĥertext
  * \param[in] sk String containing the secret key
  * return 0 if encapsulation is successful, -1 otherwise
  */
int crypto_kem_dec(unsigned char* ss, const unsigned char* ct, const unsigned char* sk) {
  ...
  #ifdef VERBOSE
    printf("\n\npk: "); for(int i = 0 ; i < PUBLIC_KEY_BYTES ; ++i) printf("%x", pk[i]);
    printf("\n\nsk: "); for(int i = 0 ; i < SECRET_KEY_BYTES ; ++i) printf("%x", sk[i]);
    printf("\n\nciphertext: "); for(int i = 0 ; i < CIPHERTEXT_BYTES ; ++i) printf("%x", sk[i]);
  #endif
  ...
  if(memcmp(d, d2, SHA512_BYTES) != 0) {
    ...
    printf("\n\nd2: "); for(int i = 0 ; i < SHA512_BYTES ; ++i) printf("%x", d2[i]);
  #endif
  ...
  sha512(ss, mc, UTILS_VEC_K_BYTES + 2 * UTILS_VECTOR_ARRAY_BYTES);
    ...
    printf("\n\nsecret2: "); for(int i = 0 ; i < SHARED_SECRET_BYTES ; ++i) printf("%x", ss[i]);
    printf("\n\n## Checking Ciphertext- End ##");
  #endif
